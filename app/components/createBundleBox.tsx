...
export function CreateBundleBox({
  ...
  // Update arrays when items change
  useEffect(() => {
    setChanceValues(new Array(mysteryItems.length).fill(0));
    setColorValues(new Array(mysteryItems.length).fill("#000000"));
  }, [mysteryItems.length]);
      ...
      <s-stack>
          ...
          <s-table-body>
            {mysteryItems.map((item, idx) => (
              <s-table-row key={idx}>
                <input
                  type="hidden"
                  name={`bundle-item-${idx}-variantId`}
                  value={item.variantId}
                />
                <input
                  type="hidden"
                  name={`bundle-item-${idx}-itemName`}
                  value={item.itemName}
                />
                <s-table-cell>
                  <s-box
        ...
        {mysteryItems.length > 0 && (
          <s-box padding="base">
            <s-stack gap="small">
              <s-text
                // type={totalChance === 100 ? "success" : "critical"}
